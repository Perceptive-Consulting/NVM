...
public class PropertyDeveloperController {
	...
	public ModelAndView uploadDocuments(HttpServletRequest request, HttpServletResponse response){
		...
		tPDPropertyObj = propertyDeveloperManager.loadPDPropertyById(propertyId);
		if(tPDPropertyObj == null || tPropertyDeveloperObj == null || loginObj == null)
			return new ModelAndView("redirect:uploadPropertyFiles");
		...
		try{
			...
			if(multiPartRequest == null)
				return new ModelAndView("user/propertyDeveloper/pdProperty/uploadPropertyFiles", "property", tPDPropertyObj);
			...
			for (MultipartFile tMultipartFileObj : multipartFileList) {
				if(tMultipartFileObj == null)
					continue;
				if(tMultipartFileObj.getOriginalFilename() == null || tMultipartFileObj.getOriginalFilename().length() == 0 || tMultipartFileObj.getSize() <= 0)
					continue;
				
				File tFileObj = new File(tMultipartFileObj.getOriginalFilename(), tMultipartFileObj.getSize(), 0, propertyId, tMultipartFileObj.getBytes(), tMultipartFileObj.getContentType(), DocumentType.IMAGES.id());
				
				tFileObj.setPropertyReference(tPDPropertyObj.getUniqueReference());
				if(param != null){
					PDDocuments documents = new PDDocuments(tPropertyDeveloperObj.getId(), propertyId, Long.valueOf(param), tMultipartFileObj.getOriginalFilename(), tFileObj);
						documentsList.add(documents);
				}
			}
			
			addOrUpdateCheck = "2";      // getCheckForUploadDocuments could be NULL for new upload
			if(tPDPropertyObj.getCheckForUploadDocuments() == null || tPDPropertyObj.getCheckForUploadDocuments().trim().length() == 0)
				addOrUpdateCheck = "1";
			//propertyDeveloperManager.loadPDPropertyDocuments(propertyDeveloperObj.getId(), tPDPropertyObj, loginObj.getId()); 
			execStatus = propertyDeveloperManager.saveDocuments(documentsList, loginObj.getId(), tPDPropertyObj,addOrUpdateCheck);
			if(execStatus){
				Boolean uploadPdDocuments = Boolean.parseBoolean("" + sessionObj.getAttribute("uploadPdDocuments"));
				
				if(addOrUpdateCheck.equals("1")) {
					sessionObj.setAttribute("uploadPdDocuments", uploadPdDocuments == false ? true : true );
					sessionObj.setAttribute("editingPDDocument", false);
				} 
				if(addOrUpdateCheck.equals("2")){
					if(uploadPdDocuments){
