  ...
  $(document).ready(function(){
	  $("#disableId").click(function(){
		  $.getJSON(contextPath+"/pmc/changeStatus.htm?value=DISABLE&pmcId="+id,
				    function(response, textStatus, jqXHR){
			  			if(response.success == true){
			  				$("#disableId").hide();
			  				$("#enableId").show();
			  				addPMCDetails(id);
			  				$("#commonMessageSuccessId").html("Successfully Reset");
	  ...
	  $("#enableId").click(function(){
		  $.getJSON(contextPath+"/pmc/changeStatus.htm?value=ENABLE&pmcId="+id,
				    function(response, textStatus, jqXHR){
			  			if(response.success == true){
			  				$("#disableId").show();
			  				$("#enableId").hide();
  ...
  $('#resetPassword').click(function(){
	  $.ajax({
	      url: contextPath+"/pmc/resetPassword?pmcId="+id,
	      type: "post",
	      dataType: "json",
	      success: function(data){
 ...
 /*----------------- load city---------------------------*/
  ...
  function loadPMCCities(){
	  ...
	  var citiesStr = $("#pmcCitiesCoveredIdStr").html();
	   ...
	   $.getJSON(contextPath+"/pmc/loadPMCCities.htm", function(options) {
	        if (options) {
	      	  var opts = options.data;
	      	var htmlStr = '<option value=""></option>';
	            $.each(opts, function(key, value) {
	          	  htmlStr += formatDropDown(this.cityId, this.city);
	            });
  ...
  function editPMCCities(){
	   ...
	   $("#pmcCityUpdateBtn").click(function(event){
		   ...
		   $.ajax({
			      url: contextPath+"/pmc/updateCities.htm",
			      type: "post",
			      dataType : "json",
			      data: {citiesStr : pmcCityStr, id : id},
