...
g_jsDatePickImagePath = getContextPath()+"/resource/css/img/";
...
JsDatePick.prototype.setConfiguration = function(a) {
	...
	this.oConfiguration.limitToToday = (a.limitToToday != null) ? a.limitToToday
			: false;
	this.oConfiguration.backDate = (a.backDate != null) ? a.backDate
			: false;
...
JsDatePick.prototype.backDateIsAvailable = function(c, a, b) {
	if (c < this.oCurrentDay.year) {
		return false
	}
	if (a < this.oCurrentDay.month && c == this.oCurrentDay.year) {
...
JsDatePick.prototype.populateMainBox = function(h) {
	...
	while (j.getMonth() == n) {
		...
		if (this.oConfiguration.backDate) {
			if (!this.backDateIsAvailable(this.currentYear, this.currentMonth,
					parseInt(j.getDate()))) {
				k = true;
				g.setAttribute("isJsDatePickDisabled", 1)
			}
